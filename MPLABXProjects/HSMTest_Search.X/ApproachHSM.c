...
static ApproachState_t CurrentState = Approach_Init;
...
ES_Event RunApproachHSM(ES_Event ThisEvent)
{
	...
	switch (CurrentState) {
	case Approach_Init: // If current state is initial Pseudo State
	...
	case Approach_Check_Right:
		if (ThisEvent.EventType != ES_NO_EVENT) { // An event is active
			switch (ThisEvent.EventType) {
			case ES_ENTRY:
				// Begin tank turning CW 45
				Drive_TankRight(MOTOR_SPEED_EXPLORE);

				ES_Timer_InitTimer(APPROACH_HSM_TIMER, MOTOR_TURN_EX_45);
				break;
			...
			case BEACON_FOUND:
				if (ThisEvent.EventParam == BEACON_FRONT) {
					// Facing beacon again
					ES_Timer_StopTimer(APPROACH_HSM_TIMER);
	...
	case Approach_Check_Left:
		if (ThisEvent.EventType != ES_NO_EVENT) {
			switch (ThisEvent.EventType) {
			case ES_ENTRY:
				// Begin turning CCW 90
	...
	case Approach_Turn180:
		...
		case ES_ENTRY:
			...
			ES_Timer_InitTimer(APPROACH_HSM_TIMER, MOTOR_TURN_EX_180 - 20);
			break;
