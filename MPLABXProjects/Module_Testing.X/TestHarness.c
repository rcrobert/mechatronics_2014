 ...
 * This will initialize the motor encoder for Port X03. It takes serial input
...
/**
 * Test harness for the MotorDriver.h library
 * w - Drive motors forward
 * s - Drive motors reverse
 * a - Turn gradual left
 * d - Turn gradual right
 * h - All stop
 * q - Turn 90 CCW
 * e - Turn 90 CW
 * v - Print battery voltage
 * r - Fast speeds used DEFAULT
 * f - Slow speeds used
 *
 */
#ifdef MOTOR_TEST
...
int TURN_TIME_MS = MOTOR_TURN_EX_180;
int TEST_SPEED = 1000;
...
int main(void)
{
    ...
    printf("\nMotor Driver test harness loaded");
	...
	if (!IsReceiveEmpty()) {
	    ...
	    case 'w':
		// Lift
		if (Drive_LiftUp(TEST_SPEED) == ERROR) {
		    printf("\nERROR");
		}
		break;
	    ...
	    case 'a':
		// Left
		if (Drive_Left(TEST_SPEED) == ERROR) {
		    printf("\nERROR");
		}
		break;
...
//		break;
	    ...
	    case 'd':
		// Right
		if (Drive_Right(TEST_SPEED) == ERROR) {
		    printf("\nERROR");
		}
		break;
	    ...
	    case 'q':
		// TLeft
		if (Drive_TankLeft(TEST_SPEED) == ERROR) {
		    printf("\nERROR");
		}

		InitTimer(1, TURN_TIME_MS);
		break;

	    case 'e':
		// TRight
		if (Drive_TankRight(TEST_SPEED) == ERROR) {
		    printf("\nERROR");
		}
	    ...
	    case 'v':
		// Voltage
		printf("\nVoltage: %5.3fV", ((float)AD_ReadADPin(BAT_VOLTAGE) /
			1023.0 * 33.0));
		break;

	    case 'r':
		TEST_SPEED = MOTOR_SPEED_EXPLORE;
		break;

	    case 'f':
		TEST_SPEED = MOTOR_SPEED_CRAWL;
		break;
...
/**
 * Test harness for the EventChecker.h library
 * Continuously calls the CheckSensors function. By defining EVENTCHECKER_TEST
 * in the project, debug printouts will be generated from within the library.
 */
...
int main(void)
{
    ...
    printf("\nEvent Checker test harness loaded");
